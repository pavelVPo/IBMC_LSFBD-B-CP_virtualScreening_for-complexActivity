...
1. Using PASS and ChEMBL data the classifiers were build to predict the propensity for chemical compounds to be tested against the about 5k biological targets from various organisms.
...
The results of the diversity assessement conducted using TDV for compounds tested against Smo are *on top*. The diversity is OK, but lower than expected from the random set (The higher blue bars on the left side - the higher diversity, the higher blue bars on the right side - the lower diversity; expected levels are depicted via horizontal lines).
...
Overall, judging by these results, it is possible to make a conclusion that one round of the in silico selection of chemical compounds for testing does not affect the diversity drastically, however, in the case of the itterative continuous selection monitoring and appropriate measures are probably needed, since the fraction of the simple scaffolds grows among the overrepresented scaffolds.

5. ChEMBL compounds studied as Smo inhibitors and ChEMBL & ChemRar compounds propense to be evaluated this way on the level of MNA-descriptors.

MNA-descriptors are used internally in PASS to represent chemical structures (https://pubs.acs.org/doi/abs/10.1021/ci980335o), also these descriptors were used to place chemical structures of the aforementioned compounds on the plane to get insight, whether datasets are mutually comparable. 

![alt text](https://github.com/pavelVPo/IBMC_LSFBD-B-CP_virtualScreening_for-complexActivity/blob/main/extUMAP_propensity.png)
